...
export const createMentor = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Mentor Create Successfully", data: mentor });
  } catch (error) {
      console.log(error);
...
export const createStudent = async (req, res) => {
  try {
    ...
    res
       .status(200)
      .json({ message: "Student Create Successfully", data: student });
  } catch (error) {
      console.log(error);
      res.status(500).json({ message: "student already exists in this email" });
...
export const addStudents = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(studentValues)) {
      res.status(404).json({ message: "Values  not a  array" });
    }
    ...
    const result = toAssign
      .filter((e) => studentValues.includes(e._id + ""))
      .map((e) => e._id);
    ...
    await Student.updateMany(
      { _id: { $in: result } },
      { $set: { mentor: selectedMentor } }
    );

    res.status(200).json({ data: selectedMentor, NotAdded: already });
  } catch (error) {
    console.log(error);
    res.status(404).json({ data: "error in operation" });
  }
};
...
export const assignMentor = async (req, res) => {
  try {
    ...
    if (!stud) {
      res.status(404).json({ message: "No Student in this ID" });
    }
    ...
    res
      .status(404)
      .json({ error: err, message: " data updated successfully" });
    // console.log(error)
  }
};
...
export const getStudents = async (req, res) => {
  try {
    const students = await Student.find(
      {},
      { _id: 1, studentName: 1, studentEmail: 1, course: 1, mentor: 1 }
    ).populate("mentor");
    if (!students) {
      res.status(404).json({ message: "error  students data" });
    }
...
export const getStudentsOfMentor = async (req, res) => {
  try {
    const mentorId = req.params.id;
    const result = await Mentor.findById(mentorId).populate({
      path: "students",
      select: "studentName",
    });
    if (!result) {
    ...
    res
      .status(404)
      .json({ message: "Could not get the Students from the Mentor ID" });
  }
};
